...
console.log('\n[After 5 failed attempts the username is locked (429)]');
{
  ...
  check(
    'first 5 attempts all 401',
    first5.every((s) => s === 401),
    `(seq=${first5.join(',')})`
  );
...
console.log('\n[Lockout does not reveal whether the username exists]');
{
  ...
  check(
    'existing user sequence == non-existent user sequence',
    JSON.stringify(existing) === JSON.stringify(missing),
    `(existing=${existing.join(',')} missing=${missing.join(',')})`
  );
  ...
  check(
    '429 bodies identical for existing vs missing',
...
console.log('\n[A successful login clears the counter]');
{
  ...
  check(
    'post-success failures not immediately locked',
    again.every((s) => s === 401),
    `(seq=${again.join(',')})`
  );
}
