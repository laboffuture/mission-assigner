...
async function completeOne(studentId, wrong) {
  ...
  const opened = await (
    await fetch(`${BASE}/api/slot/${slot.slot_id}/open`, { method: 'POST', headers: { Cookie: cookie } })
  ).json();
...
console.log('\n[A student reviews their own completed mission]');
{
  const { status, body } = await j(
    await fetch(`${BASE}/api/assignment/${s1.assignmentId}/review`, { headers: { Cookie: s1.cookie } })
  );
...
{
  const { body } = await j(
    await fetch(`${BASE}/api/assignment/${s2.assignmentId}/review`, { headers: { Cookie: s2.cookie } })
  );
...
console.log('\n[Edge cases]');
{
  ...
  const slot = wk.slots.find((s) => s.status === 'open');
  const opened = await (
    await fetch(`${BASE}/api/slot/${slot.slot_id}/open`, { method: 'POST', headers: { Cookie: cookie3 } })
  ).json();
  const notGraded = await fetch(`${BASE}/api/assignment/${opened.assignment_id}/review`, {
