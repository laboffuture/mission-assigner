...
import { seedDb, loginAs, setFeedbackGating, spreadFrom, titleFor } from './helpers';
...
// The second test reviews the mission the first completed, so each id runs as a pair, in order.
for (const id of spreadFrom(1)) {
  test(titleFor('a completed mission can be reviewed from the week board', id, 1), async ({ page }) => {
  ...
  test(titleFor('the week board marks a completed slot as reviewable', id, 1), async ({ page }) => {
    await loginAs(page, id);
    await page.goto('/week');
