import type { Segment, Slot } from '@/lib/api/types';
import { slotState, formatWeekStart } from '@/lib/week';
import { Card } from '@/components/ui';
import { PositionLine } from '@/components/CurriculumPosition';
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card className="p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-text-secondary">Week of {formatWeekStart(weekStart)}</p>
            {/* Track / credit / project / session — or the segment placement
                when curriculum mode has not placed this student. */}
            <PositionLine segment={segment} className="mt-1" />
          </div>
      ...
      <section aria-labelledby="daily-heading">
        <h2 id="daily-heading" className="mb-2 text-sm font-bold uppercase tracking-wide text-text-secondary">
          Daily sequence
        </h2>
        {dailySlots.length === 0 ? (
          <p className="text-sm text-text-secondary">No daily missions this week.</p>
        ) : (
