...
import { setSelectionMode } from './src/config.js';
...
async function newStudent(name, tz, courses = []) {
  const [r] = await db.query(
    `INSERT INTO students (display_name, age, subject, current_level, placement_status, timezone)
     VALUES (?,13,'Computer Science',0,'complete',?)`,
    [name, tz]
  );
  const id = r.insertId;
...
console.log('\n[time_to_submit_seconds is correct regardless of server timezone (computed in SQL)]');
{
  ...
  await db.query(`UPDATE assignments SET opened_at = DATE_SUB(UTC_TIMESTAMP(), INTERVAL 7 SECOND) WHERE id=?`, [
    fill.assignmentId,
  ]);
  ...
  check(
    'grade result reports the same value',
    res.timeToSubmitSeconds === Number(t),
    `(res=${res.timeToSubmitSeconds})`
  );
}
...
console.log("\n[The streak boundary is the student's midnight, not UTC midnight]");
{
  ...
  for (const expr of instants) {
    await db.query(
      `INSERT INTO attempt_logs (assignment_id, student_id, event, created_at) VALUES (?, ?, 'submitted', ${expr})`,
      [aidK, K]
    );
    await db.query(
      `INSERT INTO attempt_logs (assignment_id, student_id, event, created_at) VALUES (?, ?, 'submitted', ${expr})`,
