...
check(
  'body.error.requestId matches the header',
  boom.body?.error?.requestId === boom.reqId,
  `(header=${boom.reqId}, body=${boom.body?.error?.requestId})`
);
check(
  'generic message (no internal detail leaked)',
  boom.body?.error?.message === 'internal server error',
  `(msg=${boom.body?.error?.message})`
);
check(
  'response body carries NO stack trace',
  !/\bat\s+\w/.test(boom.text) && !boom.text.toLowerCase().includes('stack'),
...
check(
  'error logged server-side WITH stack + requestId',
  !!errEntry &&
    errEntry.requestId === boom.reqId &&
    !!errEntry.err &&
    typeof errEntry.err.stack === 'string' &&
    errEntry.err.stack.length > 0,
  `(hasErr=${!!errEntry?.err})`
);
const doneEntry = (entries || []).find((e) => e.req && e.res);
check(
  'request completion logged: method/status/duration/requestId',
  !!doneEntry &&
    doneEntry.req.method === 'GET' &&
    typeof doneEntry.res.statusCode === 'number' &&
    typeof doneEntry.responseTime === 'number' &&
    doneEntry.requestId === boom.reqId,
  `(status=${doneEntry?.res?.statusCode}, ms=${doneEntry?.responseTime})`
);

console.log('\n[Redaction: secrets never reach the logs]');
const authed = await raw('/api/feedback/questions', {
  headers: { 'X-User-Id': '1', Authorization: 'Bearer topsecret-value' },
});
...
check(
  'Authorization header redacted in the log',
  !!done2 && done2.req.headers && done2.req.headers.authorization === '[REDACTED]',
  `(auth=${done2?.req?.headers?.authorization})`
);
check(
  'raw secret value appears in NO log line for the request',
  !JSON.stringify(logs2 || []).includes('topsecret-value')
);
