...
{
  const c = await current(2);
  track(2, (await keyFor(c.assignment_id)).missionId);
  ...
  const n = await current(2);
  track(2, (await keyFor(n.assignment_id)).missionId);
...
{
  const c1 = await current(1);
  const k1 = await keyFor(c1.assignment_id);
  track(1, k1.missionId);
  ...
  const c2 = await current(1);
  const k2 = await keyFor(c2.assignment_id);
  track(1, k2.missionId);
  ...
  check(
    'second fail STILL holds 2 -> 2 (no demotion)',
    r2.level.from === 2 && r2.level.to === 2,
    `(${r2.level.from}->${r2.level.to})`
  );
  check('reason wrong_retry_same_level', r2.level.reason === 'wrong_retry_same_level', `(${r2.level.reason})`);
  const c3 = await current(1);
  track(1, (await keyFor(c3.assignment_id)).missionId);
...
{
  const c1 = await current(3);
  const k1 = await keyFor(c1.assignment_id);
  track(3, k1.missionId);
  ...
  const c2 = await current(3);
  const k2 = await keyFor(c2.assignment_id);
  track(3, k2.missionId);
  const r2 = await submit(c2.assignment_id, wrong(k2.correct), 3);
  check(
    'second fail stays 0 (never negative)',
    r2.level.to === 0,
    `(${r2.level.from}->${r2.level.to}, reason=${r2.level.reason})`
  );
}
...
for (const sid of [1, 2, 3]) {
  const [rows] = await db.query(
    `SELECT mission_id, COUNT(*) c FROM assignments WHERE student_id = ? GROUP BY mission_id HAVING c > 1`,
    [sid]
  );
...
console.log('\n[Criterion 8] selection_log captures candidates');
{
  ...
  const [[{ logCount }]] = await db.query(
    `SELECT COUNT(*) logCount FROM selection_log WHERE chosen_mission IS NOT NULL`
  );
  check(
    'one selection_log row per assignment',
    Number(logCount) === Number(assignCount),
    `(assignments=${assignCount}, logs=${logCount})`
  );
  const [[sample]] = await db.query(
    `SELECT candidates FROM selection_log WHERE chosen_mission IS NOT NULL ORDER BY id DESC LIMIT 1`
  );
  let cand = sample.candidates;
  if (typeof cand === 'string') cand = JSON.parse(cand);
  check(
    'candidate list is non-empty and has overlap scores',
    Array.isArray(cand) && cand.length > 0 && 'overlap' in cand[0],
