...
console.log('\n[Submission log is cursor-paginated: { items, nextCursor }, no overlap, full coverage]');
{
  ...
  do {
    ...
    check(
      `page ${guard + 1}: has items[] and nextCursor key`,
      Array.isArray(body.items) && 'nextCursor' in body,
      `(n=${body.items?.length})`
    );
  ...
  check(
    '25 rows total, no overlap across pages',
    seen.length === 25 && new Set(seen).size === 25,
    `(count=${seen.length}, distinct=${new Set(seen).size})`
  );
...
console.log('\n[Other list endpoints return { items, nextCursor }]');
{
  ...
  check(
    'GET /api/missions (staff) -> items + nextCursor',
    missionsList.status === 200 && Array.isArray(missionsList.body.items) && 'nextCursor' in missionsList.body,
    `(status=${missionsList.status})`
  );
  check(
    'mission bank page respects limit',
    missionsList.body.items.length === 10 && missionsList.body.nextCursor,
    `(n=${missionsList.body.items?.length})`
  );

  const mq = await get('/api/mission-quality?limit=5', SME);
  check(
    'GET /api/mission-quality (staff) -> items + nextCursor',
    mq.status === 200 && Array.isArray(mq.body.items) && 'nextCursor' in mq.body,
    `(status=${mq.status})`
  );
}
