...
// setting; no env change or restart needed). Requires ENABLE_TEST_HOOKS.
{
  ...
  if (r.status !== 200) {
    console.error(
      `FATAL: could not disable feedback gating (status ${r.status}). ` + `Start the server with ENABLE_TEST_HOOKS=1.`
    );
    process.exit(2);
  }
...
console.log('\n[Criterion 1] assignSegment placement rules');
{
  ...
  check(
    '15 without CS-101 -> CS Foundation (fallback)',
    db2.segmentName === 'CS Foundation',
    `(got ${db2.segmentName}, reason=${db2.reason})`
  );
}
...
console.log('\n[Criterion 2] cold start = segment start_level');
{
  ...
  check(
    'level set to segment start_level (1)',
    Number(row.current_level) === 1 && res.startLevel === 1,
...
console.log('\n[Criterion 4] locked slots return NO question text');
{
  ...
  check(
    'raw JSON has no body for locked slots',
    locked.every((s) => !s.mission)
  );
}
...
let studentAWeek;
{
  ...
  check(
    'slot 2 not filled before slot 1 submitted',
    slot2Before.assignment_id == null && slot2Before.status === 'locked'
  );

  const open1 = await post(`/api/slot/${slot1.slot_id}/open`);
  check(
    'slot 1 open returns a mission',
    !!open1.assignment_id && Array.isArray(open1.options),
    `(aid=${open1.assignment_id})`
  );
  ...
  check(
    '[C8] attempt XP NOT awarded on second open',
    open1b.xp && open1b.xp.awarded === false && open1b.xp.reason === 'already_awarded'
  );
  ...
  check(
    '[C5] slot 2 filled with a mission on unlock',
    slot2After.assignment_id != null,
    `(aid=${slot2After.assignment_id})`
  );
...
console.log('\n[Criterion 6] slot 5 (offset +1) serves one level above current');
{
  ...
  for (const idx of [2, 3, 4]) {
    ...
    if (!o.assignment_id) {
      console.log(`   (slot ${idx} had no mission: ${o.message || 'gap'})`);
      continue;
    }
  ...
  const [[stu]] = await db.query(
    `SELECT current_level, max_level FROM students s LEFT JOIN segments seg ON seg.id=s.segment_id WHERE s.id=1`
  );
  const wk = await j('/api/week/1');
  ...
  if (o5.assignment_id) {
    ...
    check(
      'slot 5 difficulty = current_level+1 (clamped)',
      o5.difficulty === expected,
      `(diff=${o5.difficulty}, level=${stu.current_level}, expected=${expected})`
    );
  } else {
...
console.log('\n[Criterion 7] total_xp equals the sum of xp_events');
{
  ...
  check(
    'xp endpoint returns items + total (paginated)',
    xp.total_xp === Number(total) && Array.isArray(xp.items) && 'nextCursor' in xp
  );
  ...
  check(
    'attempt, submit, correct all awarded',
    set.has('attempt') && set.has('submit') && set.has('correct'),
    `(types=${[...set]})`
  );
}
...
console.log('\n[Criterion 9 & 10 & 11] stall -> one assistance; wrong never demotes; cap at max');
{
  ...
  for (let step = 0; step < 3; step++) {
    ...
    const open =
      wk.slots.find((x) => x.status === 'open' && x.assignment_id == null) || wk.slots.find((x) => x.status === 'open');
    ...
    if (step === 0) {
      ...
      check(
        '[DTO] submit returns the correct option key',
        r.correct_option_key === k.correct,
        `(got ${r.correct_option_key})`
      );
      check(
        '[DTO] submit returns a non-empty explanation',
        typeof r.explanation === 'string' && r.explanation.length > 0
      );
      check(
        '[DTO] submit nests level {from,to,reason}',
        typeof r.level?.from === 'number' && typeof r.level?.to === 'number' && typeof r.level?.reason === 'string'
      );
      check(
        '[DTO] submit reports xp.total_xp / points_earned',
        typeof r.xp?.total_xp === 'number' && typeof r.xp?.points_earned === 'number'
      );
      check(
        '[DTO] submit does NOT leak internal studentId',
        r.studentId === undefined && r.correctAnswer === undefined
      );
    }
  }
  ...
  check(
    '[C9] student still receives a 4th mission',
    !!fourth.assignment_id || !!fourth.empty,
    `(aid=${fourth.assignment_id})`
  );
...
console.log('\n[Criterion 13] relaxation order followed + logged when no exact match');
{
  ...
  const [shorts] = await db.query(
    `SELECT id, version FROM missions WHERE subject='Computer Science' AND mission_type='quiz' AND time_band='short' AND difficulty=1 AND status='live'`
  );
  for (const m of shorts) {
    await db.query(
      `INSERT IGNORE INTO assignments (student_id, mission_id, mission_version, level_at_assign, status) VALUES (?,?,?,?, 'graded')`,
      [s, m.id, m.version, 1]
    );
  }
  ...
  const [[log]] = await db.query(
    `SELECT filters_applied FROM selection_log WHERE student_id=? ORDER BY id DESC LIMIT 1`,
    [s]
  );
  let fa = log.filters_applied;
  if (typeof fa === 'string') fa = JSON.parse(fa);
  check(
    'relaxation logged with ordered list',
    Array.isArray(fa.relaxations) && fa.relaxations[0] === 'widen_difficulty_pm1',
    `(relaxations=${JSON.stringify(fa.relaxations)})`
  );
  check(
    'a mission was still served after relaxing',
    !!o.assignment_id || !!o.empty,
    `(aid=${o.assignment_id}, msg=${o.message || ''})`
  );
}
