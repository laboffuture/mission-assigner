...
async function openSlot1(sid) {
  ...
  const o = await (
    await fetch(`${BASE}/api/slot/${slot1.slot_id}/open`, { method: 'POST', headers: { 'X-User-Id': String(sid) } })
  ).json();
...
const countXp = async (aid, ev) =>
  Number(
    (await db.query(`SELECT COUNT(*) n FROM xp_events WHERE assignment_id=? AND event_type=?`, [aid, ev]))[0][0].n
  );
const countLevel = async (aid) =>
  Number((await db.query(`SELECT COUNT(*) n FROM level_events WHERE assignment_id=?`, [aid]))[0][0].n);
const slot2Status = async (sid) => {
  const [[row]] = await db.query(
    `SELECT ws.status FROM week_slots ws JOIN student_weeks sw ON sw.id=ws.student_week_id WHERE sw.student_id=? AND ws.slot_index=2 ORDER BY sw.id DESC LIMIT 1`,
    [sid]
  );
...
console.log('\n[Two simultaneous submits with the SAME Idempotency-Key -> one grade, one replay]');
{
  ...
  check(
    'exactly one "correct" XP event',
    (await countXp(aid, 'correct')) === 1,
    `(n=${await countXp(aid, 'correct')})`
  );
  check('exactly one level_event', (await countLevel(aid)) === 1, `(n=${await countLevel(aid)})`);
  check(
    'next slot unlocked exactly once (slot 2 open)',
    (await slot2Status(sid)) === 'open',
    `(status=${await slot2Status(sid)})`
  );
...
console.log('\n[Two simultaneous submits with NO key -> lock lets exactly one through]');
{
  ...
  check(
    'one succeeds (200), one rejected (400 not-open)',
    JSON.stringify(statuses) === JSON.stringify([200, 400]),
    `(statuses=${statuses})`
  );
  check(
    'still exactly one "submit" XP event',
    (await countXp(aid, 'submit')) === 1,
    `(n=${await countXp(aid, 'submit')})`
  );
  check(
    'still exactly one "correct" XP event',
    (await countXp(aid, 'correct')) === 1,
    `(n=${await countXp(aid, 'correct')})`
  );
